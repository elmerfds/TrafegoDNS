...
const apiGroups: EndpointGroup[] = [
  ...
  },
  {
    ...
    endpoints: [
      { method: 'GET', path: '/config', description: 'Get auth mode configuration (local, none, or oidc). Returns OIDC login URL when mode is oidc.', auth: 'none' },
      { method: 'POST', path: '/login', description: 'Login with username/password. Blocked when AUTH_MODE=oidc unless OIDC_ALLOW_LOCAL_LOGIN=true.', auth: 'none', body: { username: 'string', password: 'string' } },
      ...
      { method: 'PUT', path: '/profile', description: 'Update current user profile (email, password, avatar). Password change blocked for OIDC users.', auth: 'required', body: { email: 'string?', password: 'string?', avatar: 'string?' } },
