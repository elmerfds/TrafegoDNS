...
class DNSManager {
  ...
  async processHostnames(hostnames, containerLabels) {
    try {
      ...
      let dnsHostnames = [...hostnames]; // Start with all hostnames intended for potential processing
      ...
      if (this.config.dnsProvider === 'cloudflare' && this.config.cfTunnelEnabled) {
        ...
        try {
          ...
          if (cloudflareProvider.processTunnelHostnames) {
            // Let the provider separate tunnel hostnames from DNS hostnames
            const separationResult = await cloudflareProvider.processTunnelHostnames(hostnames, containerLabels);

            // Update the hostnames list to only include DNS hostnames
            // Ensure dnsHostnames contains only those NOT handled by the tunnel
            dnsHostnames = separationResult.dnsHostnames || []; // These are the only ones that should proceed to DNS logic
            processedTunnelHostnames = separationResult.tunnelHostnames || []; // These are handled separately

            if (processedTunnelHostnames.length > 0) { // Use processedTunnelHostnames here
              logger.info(`Processed ${processedTunnelHostnames.length} hostnames using CloudFlare Tunnel`);
            }
          }
        } catch (error) {
      ...
      // *** IMPORTANT: Iterate ONLY over dnsHostnames now ***
      for (const hostname of dnsHostnames) {
        try {
          logger.trace(`DNSManager: Processing DNS for hostname: ${hostname}`);
          this.stats.total++;
          ...
          if (!shouldManage) {
            logger.trace(`DNSManager: Skipping ${hostname} as shouldManage is false.`);
            continue;
          }
          ...
          // *** ADDED CHECK: Ensure extractDnsConfigFromLabels didn't return a tunnel marker ***
          // This acts as a safeguard if the initial separation logic missed something.
          if (recordConfig && recordConfig.useTunnel === true) {
              logger.warn(`DNSManager: Hostname ${hostname} was marked for tunnel use by extractDnsConfigFromLabels but was not filtered out earlier. Skipping DNS processing.`);
              continue; // Skip adding to DNS batch
          }

          logger.trace(`DNSManager: Adding DNS config for ${hostname} to batch: ${JSON.stringify(recordConfig)}`);
