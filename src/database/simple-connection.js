...
async function initialize() {
  ...
  initializationPromise = (async () => {
    try {
      logger.info('Initializing database connection with simplified approach');
      const success = await sqliteCore.initialize();
      
      if (success) {
        isInitialized = true;
        logger.info('Database successfully initialized with simplified approach');
        return true;
      } else {
        logger.error('Failed to initialize database with simplified approach');
        return false;
      }
    } catch (error) {
